...
void generate_matrix(double *A, int n) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i == j) {
                A[i * n + j] = (rand() % 5) + n * 5.0; // diagonal dominante
            } else {
                A[i * n + j] = (rand() % 5) - 2.0; // valores pequeños fuera de la diagonal
            }
        }
    }
}
...
void run_inverse(int N, int rank, int size) {
    ...
    if (rank == 0) {
        printf("\n--- Inversi\u00f3n de matriz aleatoria %dx%d con %d procesos ---\n", N, N, size);
        printf("%s", info);
...
int main(int argc, char **argv) {
    ...
    int base = 1000;
    int scale = (int) ceil(sqrt((double)size));
    int N = base * scale;
